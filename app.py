...
# ── Corpus path: works both on Streamlit Cloud and Kaggle ─────────────────────
KAGGLE_PATH = "/kaggle/input/competitions/niat-masterclass-rag-challenge/zyro-dynamics-hr-corpus/"
LOCAL_PATH  = os.path.join(os.path.dirname(__file__), "corpus")
...
if os.path.isdir(KAGGLE_PATH):
    ...
    st.error(
        ...
        "For **Streamlit Cloud**: add a `corpus/` folder to your GitHub repo "
        "containing all 11 HR policy PDFs, then redeploy.\n\n"
        "Expected path: `corpus/` next to `app.py`."
    )
    st.stop()
...
def build_pipeline():
    ...
    if not documents:
        raise ValueError(
            f"No PDF files found in '{CORPUS_PATH}'. "
            "Make sure the docs/ folder contains the 11 HR policy PDFs."
        )
